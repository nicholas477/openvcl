...
static void
    hash_new_table(size, ptr) int size;
hash_table* ptr;
{
	int i;
	ptr->size  = size;
	ptr->table = (hash_entry**)malloc(size * (sizeof(hash_entry*)));
...
sb* key;
{
	...
	while (p)
	{
		if (p->key.len == key->len && strncmp(p->key.ptr, key->ptr, key->len) == 0)
			return p;
		p = p->next;
	}
	return 0;
...
static void
    checkconst(op, term) int op;
exp_t* term;
{
	if (term->add_symbol.len || term->sub_symbol.len)
	{
...
exp_t* lhs;
{
	...
	while (idx < string->len && (string->ptr[idx] == '*' || string->ptr[idx] == '/'))
	{
...
sb* out;
{
	...
	if (ISFIRSTCHAR(in->ptr[i]) || in->ptr[i] == '\\')
	{
		...
		i++;
		while ((ISNEXTCHAR(in->ptr[i]) || in->ptr[i] == '\\' || in->ptr[i] == '&') && i < in->len)
		{
			sb_add_char(out, in->ptr[i]);
			i++;
		}
...
static void
    change_base(idx, in, out) int idx;
sb* in;
sb* out;
{
	char buffer[20];
	...
	while (idx < in->len)
	{
		if (in->ptr[idx] == '\\' && idx + 1 < in->len && in->ptr[idx + 1] == '(')
		{
			idx += 2;
...
change_base2(int idx, sb* in, sb* out)
{
	...
	while (idx < in->len)
	{
		...
		else if (in->ptr[idx] == '\\' && idx + 1 < in->len && in->ptr[idx + 1] == '(')
		{
			idx += 2;
			while (idx < in->len && in->ptr[idx] != ')')
			{
...
static void
    do_assign(again, idx, in) int again;
int idx;
sb* in;
{
...
sb* line;
{
	...
	if (idx < line->len && ISCOMMENTCHAR(line->ptr[idx]))
		return 1;
	if (idx >= line->len)
		return 1;
	return 0;
...
int pretend_quoted;
{
	...
	if (idx < in->len)
	{
		...
		else if (in->ptr[idx] == '%' && alternate && expand)
		{
			int val;
			char buf[20];
		...
		else if (in->ptr[idx] == '"' || in->ptr[idx] == '<' || (alternate && in->ptr[idx] == '\''))
		{
			if (alternate && expand)
			{
				/* Keep the quotes.  */
		...
		{
			while (idx < in->len && (in->ptr[idx] == '"' || in->ptr[idx] == '\'' || pretend_quoted || !ISSEP(in->ptr[idx])))
			{
...
sb* out;
{
	...
	if (len < 0 || pos < 0 ||
	    pos > string.len || pos + len > string.len)
	{
		sb_add_string(out, " ");
	}
	else
...
static void
    process_assigns(idx, in, buf) int idx;
sb* in;
sb* buf;
{
	while (idx < in->len)
	{
		...
		else if (idx + 3 < in->len && in->ptr[idx] == '.' && TOUPPER(in->ptr[idx + 1]) == 'L' && TOUPPER(in->ptr[idx + 2]) == 'E' && TOUPPER(in->ptr[idx + 3]) == 'N')
			idx = dolen(idx + 4, in, buf);
		else if (idx + 6 < in->len && in->ptr[idx] == '.' && TOUPPER(in->ptr[idx + 1]) == 'I' && TOUPPER(in->ptr[idx + 2]) == 'N' && TOUPPER(in->ptr[idx + 3]) == 'S' && TOUPPER(in->ptr[idx + 4]) == 'T' && TOUPPER(in->ptr[idx + 5]) == 'R')
			idx = doinstr(idx + 6, in, buf);
		else if (idx + 7 < in->len && in->ptr[idx] == '.' && TOUPPER(in->ptr[idx + 1]) == 'S' && TOUPPER(in->ptr[idx + 2]) == 'U' && TOUPPER(in->ptr[idx + 3]) == 'B' && TOUPPER(in->ptr[idx + 4]) == 'S' && TOUPPER(in->ptr[idx + 5]) == 'T' && TOUPPER(in->ptr[idx + 6]) == 'R')
		...
		else if (ISFIRSTCHAR(in->ptr[idx]))
		{
			...
			sb acc;
			int cur = idx + 1;
			while (cur < in->len && (ISNEXTCHAR(in->ptr[cur])))
				cur++;
...
process_file()
{
	...
	while (more)
	{
		...
		else if (mri && (line.ptr[0] == '*' || line.ptr[0] == '!'))
		{
			/* MRI line comment.  */
		...
		else
		{
			...
			if (label_in.len)
			{
				...
				do_assigns = 1;
				if (l < line.len && (line.ptr[l] == '.' || alternate || mri))
				{
					int lx = l;

					if (line.ptr[lx] == '.')
						++lx;
					if (lx + 3 <= line.len && strncasecmp("EQU", line.ptr + lx, 3) == 0 && (lx + 3 == line.len || !ISFIRSTCHAR(line.ptr[lx + 3])))
						do_assigns = 0;
					else if (lx + 6 <= line.len && strncasecmp("ASSIGN", line.ptr + lx, 6) == 0 && (lx + 6 == line.len || !ISFIRSTCHAR(line.ptr[lx + 6])))
						do_assigns = 0;
				}
...
sb* in;
{
	...
	if (!mri)
		...
		idx = sb_skip_white(idx, in);
	sb_new(&what);
	while (idx < in->len && (mri ? !eol(idx, in) : in->ptr[idx] != ')'))
	{
...
int warn;
{
	...
	while (idx < inbuf->len && ISNEXTCHAR(inbuf->ptr[idx]))
	{
...
int terminator;
{
	...
	else
	{
		...
		while (idx < in->len && in->ptr[idx] != terminator)
		{
			sb_add_char(val, in->ptr[idx]);
			++idx;
		}
...
int ifnc;
{
	...
	res = (first.len == second.len && strncmp(first.ptr, second.ptr, first.len) == 0);
	res ^= ifnc;
...
do_exitm()
{
	...
	if (type == include_repeat || type == include_while || type == include_macro)
	{
		int index = sp->index;
		include_pop();
		while (sp->index == index && sp->type == type)
		{
			include_pop();
		}
	}
...
static void
    do_local(idx, line) int idx ATTRIBUTE_UNUSED;
...
sb* acc;
{
	...
	while (idx < in->len && (in->ptr[idx] == '"' || in->ptr[idx] == '<' || (in->ptr[idx] == '\'' && alternate)))
	{
		if (in->ptr[idx] == '<')
		{
			if (alternate || mri)
...
{
	static int index;
	if (!unreasonable && index > MAX_REASONABLE)
	                          ...
	                          (int*)NULL)) != EOF)
	{
		switch (opt)
		{
			...
			case 'u':
				...
				break;
			case 'I': {
				include_path* p = (include_path*)malloc(sizeof(include_path));
				p->next         = NULL;
				sb_new(&p->path);
