...
void
    hash_die(table) struct hash_control* table;
{
...
void
    hash_traverse(table, pfn) struct hash_control* table;
...
void
    destroy(string, value) char* string;
char* value;
{
	free(string);
	free(value);
...
void
    applicatee(string, value) char* string;
char* value;
{
	printf("%.20s-%.20s\n", string, value);
}
