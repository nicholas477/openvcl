...
#define ISSEP(x) \
	((x) == ' ' || (x) == '\t' || (x) == ',' || (x) == '"' || (x) == ';' || (x) == ')' || (x) == '(' || ((macro_alternate || macro_mri) && ((x) == '<' || (x) == '>')))
/**/ /*...'a'...*/
#define ISBASE(x) \
	((x) == 'b' || (x) == 'B' || (x) == 'a' || (x) == 'A' || (x) == 'q' || (x) == 'Q' || (x) == 'h' || (x) == 'H' || (x) == 'd' || (x) == 'D')
...
void
    macro_init(alternate, mri, strip_at, expr) int alternate;
int mri;
int strip_at;
...
int(*get_line) PARAMS((sb*));
{
	...
	while (more)
	{
		...
		if (!macro_alternate && !macro_mri)
		{
			...
			/* Skip over a label.  */
			while (i < ptr->len && (ISALNUM(ptr->ptr[i]) || ptr->ptr[i] == '_' || ptr->ptr[i] == '$'))
				i++;
		...
		if (i < ptr->len && (ptr->ptr[i] == prefix_char || macro_alternate || macro_mri))
		{
			if (ptr->ptr[i] == prefix_char)
				i++;
			if (strncasecmp(ptr->ptr + i, from, from_len) == 0 && (ptr->len == (i + from_len) || !ISALNUM(ptr->ptr[i + from_len])))
				depth++;
			if (strncasecmp(ptr->ptr + i, to, to_len) == 0 && (ptr->len == (i + to_len) || !ISALNUM(ptr->ptr[i + to_len])))
			{
				depth--;
				if (depth == 0)
				{
...
sb* name;
{
	if (idx < in->len && (ISALPHA(in->ptr[idx]) || in->ptr[idx] == '_' || in->ptr[idx] == '$'))
	{
		sb_add_char(name, in->ptr[idx++]);
		while (idx < in->len && (ISALNUM(in->ptr[idx]) || in->ptr[idx] == '_' || in->ptr[idx] == '$'))
		{
			sb_add_char(name, in->ptr[idx++]);
		}
	}
...
sb* acc;
{
	...
	while (idx < in->len && (in->ptr[idx] == '"' || (in->ptr[idx] == '<' && (macro_alternate || macro_mri)) || (in->ptr[idx] == '\'' && macro_alternate)))
	{
		if (in->ptr[idx] == '<')
		{
			int nest = 0;
...
int pretend_quoted;
{
	...
	if (idx < in->len)
	{
		...
		else if (in->ptr[idx] == '"' || (in->ptr[idx] == '<' && (macro_alternate || macro_mri)) || (macro_alternate && in->ptr[idx] == '\''))
		{
		...
		{
			while (idx < in->len && (in->ptr[idx] == '"' || in->ptr[idx] == '\'' || pretend_quoted || (in->ptr[idx] != ' ' && in->ptr[idx] != '\t' && in->ptr[idx] != ',' && (in->ptr[idx] != '<' || (!macro_alternate && !macro_mri)))))
			{
				if (in->ptr[idx] == '"' || in->ptr[idx] == '\'')
				{
					char tchar = in->ptr[idx];
...
sb* in;
{
	...
	while (idx < in->len)
	{
		...
		formal = (formal_entry*)malloc(sizeof(formal_entry));
...
const char** namep;
{
	...
	macro = (macro_entry*)malloc(sizeof(macro_entry));
	sb_new(&macro->sub);
	sb_new(&name);
...
{
	idx = get_token(idx, in, name);
	if (idx < in->len && in->ptr[idx] == kind && (!macro_mri || macro_strip_at) && (!macro_strip_at || kind == '@'))
		idx++;
	return idx;
}
...
int copyifnotthere;
{
	...
	if (macro_strip_at && kind == '@' && (src == start || in->ptr[src - 1] != '@'))
		ptr = NULL;
	else
...
int locals;
{
	...
	while (src < in->len)
	{
		...
		else if ((macro_alternate || macro_mri) && (ISALPHA(in->ptr[src]) || in->ptr[src] == '_' || in->ptr[src] == '$') && (!inquote || !macro_strip_at || (src > 0 && in->ptr[src - 1] == '@')))
		{
			if (!locals || src + 5 >= in->len || strncasecmp(in->ptr + src, "LOCAL", 5) != 0 || !ISWHITE(in->ptr[src + 5]))
			{
				sb_reset(&t);
			...
			else
			{
				...
				while (in->ptr[src] != '\n' && in->ptr[src] != comment_char)
				{
					...
					f = (formal_entry*)malloc(sizeof(formal_entry));
					sb_new(&f->name);
					sb_new(&f->def);
					sb_new(&f->actual);
					f->index = LOCAL_INDEX;
		...
		else if (comment_char != '\0' && in->ptr[src] == comment_char && src + 1 < in->len && in->ptr[src + 1] == comment_char && !inquote)
		{
		...
		else if (macro_mri && in->ptr[src] == '=' && src + 1 < in->len && in->ptr[src + 1] == '=')
		{
			formal_entry* ptr;
...
int comment_char;
{
	...
	if (macro_mri)
	{
		...
		if (idx < in->len && in->ptr[idx] == '.')
		{
			...
			if (idx < in->len && in->ptr[idx] != ' ' && in->ptr[idx] != '\t')
			{
				...
				n = (formal_entry*)malloc(sizeof(formal_entry));
				sb_new(&n->name);
				sb_new(&n->def);
				sb_new(&n->actual);
				n->index = QUAL_INDEX;
	...
	while (idx < in->len && in->ptr[idx] != comment_char)
	{
		...
		scan = idx;
		while (scan < in->len && !ISSEP(in->ptr[scan]) && !(macro_mri && in->ptr[scan] == '\'') && (!macro_alternate && in->ptr[scan] != '='))
			scan++;
...
int comment_char;
{
	...
	while (idx < in->len && (in->ptr[idx] != comment_char))
	{
		...
		/* Look and see if it's a positional or keyword arg.  */
		scan = idx;
		while (scan < in->len && !ISSEP(in->ptr[scan]) && !(macro_mri && in->ptr[scan] == '\'') && (!macro_alternate && in->ptr[scan] != '=') && (in->ptr[scan] != comment_char))
			scan++;
...
macro_entry** info;
{
	...
	if (!ISALPHA(*line) && *line != '_' && *line != '$' && (!macro_mri || *line != '.'))
		return 0;
